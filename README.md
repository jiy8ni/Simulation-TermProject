...
이 문서와 결과 파일에 반복해서 나오는 말들입니다. 처음 보는 분은 여기부터 읽으세요.

| 용어 | 쉬운 뜻 |
| --- | --- |
| **진입로** (leg) | 사거리로 **들어오는 한쪽 도로**. 사거리니까 한 교차로에 4개(동·서·남·북). |
| **이동류** (movement) | "어느 진입로에서 + 어느 방향(좌회전/직진/우회전)으로" 가는 **차량 무리**. |
| **회전 비율** | 한 진입로에서 좌회전/직진/우회전으로 가는 차의 비율. |
| **신호 시차** (offset) | 두 교차로 초록불이 **켜지는 시점 차이**. 잘 맞으면 차가 안 멈추고 지나감(연동). |
| **연동 구간** | 두 교차로 신호를 맞춰 차가 잘 흐르도록 함께 보는 구간(여기선 교차로 15+16). |
| **1단계 모델** (Tier 1) | Arena 모델의 **가장 단순·확실한 버전** (자세한 건 Arena 가이드 참고). |
...
또한 **교차로 15·16을 함께 돌리는 Arena 모델(1단계) 입력**도 준비됐습니다. 차량 출발지를
사거리의 물리 진입로 4개로 묶어, 진입로별 도착률·회전 비율·차로 수·신호 스케줄·화면 배치 좌표까지
만들어 두었습니다. **Arena를 어떻게 조립하는지**는 [reports/arena_model_guideline.md](reports/arena_model_guideline.md)에
처음 보는 사람도 따라 할 수 있게 정리했습니다.
...
6. [data_processed/movement_ratio.csv](data_processed/movement_ratio.csv)
   - 유입 방향별 직진/좌회전/우회전 비율 (개별 폴더 기반, `unknown` 2.4~6.7%)
...
### 12. 회전 비율 소스 분리 (movement_ratio 재생성)

- 스크립트: [src/12_movement_ratio_individual.py](src/12_movement_ratio_individual.py)
- 결과:
  - [data_processed/movement_ratio.csv](data_processed/movement_ratio.csv) (개별 폴더 기반, 정식본)
  - [data_processed/movement_ratio_combined_legacy.csv](data_processed/movement_ratio_combined_legacy.csv) (기존 통합 기반 백업)

6단계에서 만든 movement_ratio는 통합 폴더 기반이라 회전 `unknown`이 43.5%였습니다.
...
| `movement_ratio.csv` | 이동 방향 비율 (개별 폴더 기반, unknown 2.4~6.7%) |
| `movement_ratio_combined_legacy.csv` | 옛 통합 기반 이동 방향 비율 (unknown 43.5%, 비교용) |
...
### 회전방향 `unknown` 대폭 감소 (소스 분리)

- 기존 [data_processed/movement_ratio.csv](data_processed/movement_ratio.csv)는 회전방향
  `unknown`이 **약 43.5%** 였습니다.
- **원인:** vehicle_core가 회전 라벨을 **통합 폴더(`교차로_15_16`)** 에서만 가져왔는데,
  그 폴더는 회전 라벨이 절반 가까이(48%/41%) 비어 있었습니다. (대기열 차량까지 담지만
  회전 분류가 안 된 데이터) 같은 교차로·시간대라도 **개별 폴더**(`교차로_15`, `교차로_16`)에는
  회전 라벨이 거의 완비되어 있습니다(빈값 2.5~7%).
- **해결(소스 분리):** 차량 수·도착률·검증 지표는 통합 폴더(완전한 모집단) 그대로 두고,
  **회전 '비율'만 개별 폴더에서 재산출**했습니다. movement_ratio는 접근로별 직진/좌/우 비율이라
  count 모집단과 일치할 필요가 없어, 깨끗한 개별 폴더로 비율만 정확히 뽑으면 됩니다.
- **결과:** `unknown` 43.5% → **교차로15 2.4% / 교차로16 6.7% / 15+16 4.9%**.
- 스크립트: [src/12_movement_ratio_individual.py](src/12_movement_ratio_individual.py).
